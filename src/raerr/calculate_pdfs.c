...
int calculate_pdfs(struct radar *radar_data, struct errorstats *error_statistics){
		...
		*(radar_data->nearest_stn + pix)	= (int)closest_stn;
		...
		for (percentile=0; percentile<100; ++percentile){
			if (*(radar_data->rain_rate+pix) < 1){
				 ...
				 */
				error	= find_percentile_error(((double)percentile)/100+0.005, mu, sigma);
			}
			*(*(radar_data->rain_rate_pdf+pix)+percentile) = *(radar_data->rain_rate+pix)/error;
		}
	}
