;2/18/2008 added boundary array

pro caldev,du0,kt,ks,boundary,nstu,ntu,np,nvu,devm2

du=du0*0 - 9999.
for i=0,ntu-1 do begin
  kb=boundary[1,i]
  kt=boundary[2,i]
  du[*,kb:kt,*,i] = du0[*,kb:kt,*,i]
endfor
;---------------

devm=fltarr(nvu,np)
dev =du*0.0
      ;1,3,4,5 (q*Lv0/Cpd,u,v,T)
	Lv0=2.5E+6
	cpd=1005.7
 
 for m=1,5 do begin
  for k=0,np-1 do begin
     ddj = reform(du[m,k,*,1:ntu-2])                    ;!! new
     jj=where(abs(ddj) lt 9990.,cnt)                    ;!!
     if(cnt gt 0)then devm[m,k] = mean(ddj[jj])                          ;!! 
 ;;;    devm(m,k)=total(du(m,k,*,1:ntu-2))/nstu/(ntu-2)  ;old before 2/18/2008
  endfor
 endfor

 for i=1,ntu-2 do begin
  for ist=0,nstu-1 do begin
   dev(*,*,ist,i)=(du(*,*,ist,i)-devm(*,*))
  endfor
 endfor
 ...
 for m=1,5 do begin
  for k=ks,kt do begin
     ddj = reform(dev[m,k,*,1:ntu-2])                    ;!! new
     jj=where(abs(ddj) lt 9990.,cnt)                    ;!!
     ddj2=ddj[jj]
     devm2[m,k] = sqrt( mean(ddj2^2) )                          ;!! 
     ;;devm2(m,k)=sqrt(total(dev(m,k,*,1:ntu-2)^2)/nstu/(ntu-2))  ;old
  endfor
 endfor

 devm2(1,*)=devm2(1,*)/Lv0*Cpd

end

;
;The following was used before 2/18/2008
;
pro caldev_bef02182008,du,kt,ks,nstu,ntu,np,nvu,devm2
...
dev =du*0.0
 ...
 for m=1,5 do begin
  for k=ks,kt do begin
     devm(m,k)=total(du(m,k,*,1:ntu-2))/nstu/(ntu-2)
  endfor
 endfor
 ...
 for m=1,5 do begin
  for k=ks,kt do begin
     devm2(m,k)=sqrt(total(dev(m,k,*,1:ntu-2)^2)/nstu/(ntu-2))
  endfor
 endfor
...
;xie	    nr_ludcmp,a1,index
;xie            ld1=nr_lubksb(a1,index,b1)
	    ludc,a1,index
            ld1=lusol(a1,index,b1)
;;	    print,'ld1'
;;	    print,ld1
;;	    print,'b2'
	    print,'b2',a2#ld1
...
; 12/30/99
; to update T and Z based on s,r
	  ...
	  for ist=0,nstu-1 do begin
              ...
              zsc = dss(6,ist)
              sc  = reform(dus(2,*,ist))
              rc  =reform(dus(1,*,ist))*Cpd/Lv0
              rc(0) = dss(1,ist)*cpd/Lv0
              sc(0) = dss(2,ist) 
              s_r_to_T_z,p,psc,zsc,sc,rc,Tc,zc
